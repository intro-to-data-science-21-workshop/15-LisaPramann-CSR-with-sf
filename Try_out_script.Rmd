...
redbridge_centroid = st_centroid(redbridge) #gives you central point in selected area
...
```{r}
plot_sf(world["continent"], reset = FALSE)
cex = sqrt(world$pop) / 10000
world_cents = st_centroid(world, of_largest = TRUE)
plot_sf(st_geometry(world_cents), add = TRUE, cex = cex)


Germany = world[world$name_long == "Germany", ]
plot_sf(st_geometry(Germany), expandBB = c(0, 0.2, 0.1, 1), col = "orange", lwd = 3)
plot_sf(Europe[0], add = TRUE)
...
us_income = left_join(us_states, us_states_df,
                      by = c("NAME" = "state"))
class(us_income)
plot_sf(us_income["median_income_15"])

# Use Joins
#Example 1
Population = left_join(world, worldbank_df)
class(Population)
plot_sf(Population["pop_growth"])
