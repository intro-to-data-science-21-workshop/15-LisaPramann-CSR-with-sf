...
CRS_vector <- st_set_crs(vector, "EPSG:4326") # set CRS
```
```{r, include=FALSE}
ggplot() +
...
```
```{r, eval = TRUE, warning = FALSE}
Globe_vector <- st_set_crs(CRS_vector, "EPSG:2163") # set CRS
```
```{r, include=FALSE}
ggplot() +
geom_sf(data = Globe_vector) + #For simple plots, you will only need geom_sf() 
