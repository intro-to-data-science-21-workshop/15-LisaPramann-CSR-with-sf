...
vector <- read_sf(vector_filepath)
```
...
```{r, eval = TRUE}
st_crs(vector)$IsGeographic #to check if the CRS is geographic or not
st_crs(vector)$units_gdal #to find out the CRS units
st_crs(vector)$srid #extracts its SRID (projection code common with GIS)
st_crs(vector)$proj4string #extracts the proj4string representation
```
---
#Key Features (3)
## 2 | Add or Change CRS
...
- CRS is used in combination with other tools to visualize public policy by mapping certain data e.g. median income, voting behavior, COVID-vaccination rates, etc. 

- Continue to build on the skills in this session and have a look at how to use this for data visualization purposes 
